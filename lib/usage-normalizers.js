...
function hasAnthropicCompatibleUsage(payload = {}) {
  const usage = payload.usage || payload.message?.usage || {};
  return (
...
export function normalizeClaudeMicuUsage(payload = {}, fallbackModel) {
  if (hasAnthropicCompatibleUsage(payload)) {
    return normalizeAnthropicCompatibleUsage(payload, fallbackModel, 'claude-micu');
  }
...
export function normalizeUsageByProvider(provider, payload = {}, fallbackModel) {
  ...
  switch (normalizedProvider) {
    ...
    case 'claude-micu':
    case 'micu-claude':
      return normalizeClaudeMicuUsage(payload, fallbackModel);
    case 'claude-yunwu-float':
      return normalizeOpenAIUsage(payload, fallbackModel, 'claude-yunwu-float');
    case 'openai':
    case 'gpt':
